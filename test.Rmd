...
```{r}
headways <- get_stop_frequency(gtfs, start_time = "06:00:00", end_time = "22:00:00", service_ids = gtfs$.$dates_services$service_id)
...
leaflet() %>%
  addTiles() %>%
  setView(lng = -80.8431, lat = 35.2271, zoom = 14) %>%
  addMarkers(lng = stops$stop_lon, lat = stops$stop_lat, popup = stops$stop_name) %>%
  addPolylines(data = shapes_sf, color = "blue", stroke = 0.5, opacity = 0.5)
```
